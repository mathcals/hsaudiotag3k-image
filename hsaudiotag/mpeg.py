...
BR_M1_L1 = (0, 32, 64, 96, 128, 160, 192, 224,
            256, 288, 320, 352, 384, 416, 448, 0)
BR_M1_L2 = (0, 32, 48, 56, 64, 80, 96, 112, 128,
            160, 192, 224, 256, 320, 384, 0)
BR_M1_L3 = (0, 32, 40, 48, 56, 64, 80, 96, 112,
            128, 160, 192, 224, 256, 320, 0)
BR_M2_L1 = (0, 32, 48, 56, 64, 80, 96, 112, 128,
...
class FrameBrowser:
    ...
    def _seek(self):
        ...
        while (index > -1):
            try:
                result = MpegFrameHeader(
                    unpack('!I', data[index:index+HEADER_SIZE])[0])
                if result.valid:
                    nextindex = index + result.size
                    try:
                        next = MpegFrameHeader(
                            unpack('!I', data[nextindex:nextindex+HEADER_SIZE])[0])
                        if next.valid:
                            self.position += index
                            self.frame = result
                            return True
...
class Mpeg:
    ...
    def bitrate(self):
        if self.vbr and (self.vbr.frames > 0):
            coeff = get_vbr_coefficient(
                self._frameheader.mpeg_id, self._frameheader.layer)
