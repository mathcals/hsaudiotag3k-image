...
class WMADecoder(object):
    ...
    def _read_file_prop(self, data):
        data.seek(48)
        play_time1, play_time2 = unpack(
            '<2I', data.read(8))  # in 100-nanosec increment
    ...
    def _read_stream_prop(self, data):
        data.seek(60)
        self.channels, self.sample_rate, self._avg_bytes_per_second = unpack(
            "<hii", data.read(10))
    ...
    def _read_content_desc(self, data):
        ...
        fields = [self._decode_string(
            data.read(size)) if size > 0 else '' for size in sizes]
    ...
    def _read_ext_content(self, data):
        ...
        for i in range(field_count):
            ...
            try:
                field_name = self._decode_string(
                    data.read(name_size)).encode().upper()
            ...
            if fp.read(WMA_ID_SIZE) == WMA_HEADER_ID:
                ...
                for i in range(item_count):
                    ...
                    if item_id in functions:
                        functions[item_id](
                            BytesIO(fp.read(item_size - WMA_OB_HEADER_SIZE)))
                    else:
                ...
                if fp.read(WMA_ID_SIZE) == WMA_DATA_ID:
                    self.audio_size = unpack("<i", fp.read(4))[
                        0] - WMA_OB_HEADER_SIZE
